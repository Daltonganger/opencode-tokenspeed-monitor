...
import { activateHubDevice, bulkSetHubDevicesStatus, cleanupExpiredNonces, getHubDevice, getHubModels, getHubProjects, getHubProviders, getHubSummary, getHubTimeseries, isNonceUsed, listHubDevices, openHubDatabase, registerHubDevice, revokeHubDevice, storeNonce, touchHubDeviceSeen, upsertHubBuckets, } from "./database";
...
function isRegisterPayload(input) {
    ...
    if (candidate.deviceId !== undefined && typeof candidate.deviceId !== "string")
        return false;
    if (candidate.anonUserId !== undefined && typeof candidate.anonUserId !== "string")
        return false;
    if (candidate.label !== undefined && typeof candidate.label !== "string")
        return false;
    return true;
}
function isBootstrapPayload(input) {
    if (typeof input !== "object" || input === null)
        return false;
    const candidate = input;
    if (candidate.deviceId !== undefined && typeof candidate.deviceId !== "string")
    ...
    const serve = (port) => Bun.serve({
        ...
        routes: {
            ...
            "/assets/tokenspeed-logo.webp": (req) => {
                ...
                if (!logo)
                    return err(404, "Logo not found");
                return withCors(new Response(logo, {
                    headers: {
            ...
            "/v1/devices/bootstrap": async (req) => {
                ...
                if (parsedBody instanceof Response)
                    return parsedBody;
                if (!isBootstrapPayload(parsedBody))
                    return err(400, "Invalid bootstrap payload");
                const deviceId = parsedBody.deviceId?.trim() || randomDeviceId();
                const anonUserId = parsedBody.anonUserId?.trim() || deviceId;
                const label = parsedBody.label?.trim() || null;
                const device = registerHubDevice(db, deviceId, label, anonUserId);
            ...
            "/v1/devices": (req) => {
                ...
                return json(listHubDevices(db, limit, { status, deviceId, anonUserId }).map((device) => ({
                    deviceId: device.deviceId,
                    anonUserId: device.anonUserId,
                    label: device.label,
                    status: device.status,
