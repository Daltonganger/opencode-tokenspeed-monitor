...
export function resolveOpenCodeHome(): string {
  ...
  if (configured) return resolvePathFromEnv(configured);
...
export function resolveDatabasePath(dbPath?: string): string {
  ...
  if (direct) return resolvePathFromEnv(direct);

  const envPath = process.env.TS_DB_PATH?.trim();
  if (envPath) return resolvePathFromEnv(envPath);

  return resolveDefaultDatabasePath();
}

function maybeMigrateLegacyDatabase(targetPath: string): void {
  if (process.env.TS_DB_PATH?.trim()) return;
